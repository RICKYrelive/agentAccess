<template>
  <div class="h-full flex flex-col bg-white">
    ...
    <div class="px-6 py-4 border-b border-slate-200">
      <div class="flex items-center justify-between mb-2">
        ...
        <div class="flex items-center gap-2">
          <!-- Edit Config Button -->
          <button
            @click="showEditConfigDialog = true"
    ...
    <!-- Edit Config Dialog -->
    <EditConfigDialog
      v-model:open="showEditConfigDialog"
      :current-config="getCurrentConfig()"
      @save="handleEditConfigSave"
    />

    <!-- Create Template Dialog -->
    <CreateTemplateDialog
      v-model:open="showCreateTemplateDialog"
      :current-config="getCurrentConfig()"
    />
  </div>
</template>
...
import EditConfigDialog from '../access-template/EditConfigDialog.vue'
import CreateTemplateDialog from '../access-template/CreateTemplateDialog.vue'
...
const handleEditConfigSave = async (config: any) => {
  ...
  // Re-send the last message with new config
  const lastUserMessage = [...currentMessages.value]
    .reverse()
    .find(m => m.role === 'user')

  if (lastUserMessage) {
    await chatStore.sendMessage(lastUserMessage.content, config)
  }
}
