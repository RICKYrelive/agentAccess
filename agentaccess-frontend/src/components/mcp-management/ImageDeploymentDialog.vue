  ...
  <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" @click.self="$emit('close')">
    <div class="bg-white rounded-lg shadow-xl w-full max-w-md">
      ...
      <div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
        <h3 class="text-lg font-semibold text-gray-900">{{ tool ? '编辑' : '导入' }} MCP 镜像</h3>
      ...
      <div class="px-6 py-4">
        <form @submit.prevent="handleSubmit" class="space-y-4">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">镜像类型</label>
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">包名 *</label>
            <input
          ...
          <div v-if="formData.packageName" class="p-3 bg-gray-50 rounded-md border border-gray-200">
            <p class="text-xs text-gray-500 mb-1">镜像信息 (预览)</p>
            <p class="text-sm font-medium text-gray-900">{{ formData.packageName }}</p>
            <p class="text-xs text-gray-500">{{ formData.version || 'latest' }}</p>
            <p class="text-xs text-gray-400 mt-1">作者: Unknown • 镜像来源: {{ formData.packageType === 'npx' ? 'npm' : 'PyPI' }}</p>
          </div>
        </form>
      </div>
