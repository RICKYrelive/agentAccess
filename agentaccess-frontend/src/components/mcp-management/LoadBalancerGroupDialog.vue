<template>
  <Teleport to="body">
    <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[60]" @click.self="handleOverlayClick">
      <div class="bg-white rounded-lg shadow-xl w-full max-w-md max-h-[90vh] overflow-hidden" @click.stop>
        <!-- Header -->
        ...
        <form @submit.prevent="handleSubmit" @click.stop class="flex flex-col h-full">
          <div class="px-6 py-4 overflow-y-auto flex-1 max-h-[60vh]">
            <div class="space-y-4">
              <!-- Group Name -->
              <div>
              ...
              <!-- Tool Selection -->
              <div>
                <label class="block text-sm font-medium text-slate-700 mb-3">选择工具</label>
              ...
              <div class="border-t border-slate-200 pt-4">
                <div class="flex items-center justify-between mb-3">
                  ...
                  <button
                    ...
                    :class="[
                      'relative inline-flex h-6 w-11 transition-colors focus:outline-none focus:ring-2 focus:ring-primary-500 rounded-full',
                      formData.healthCheckEnabled ? 'bg-primary-600' : 'bg-slate-200',
                    ]"
                  >
                    <span
                      :class="[
                        'inline-block h-4 w-4 transform rounded-full bg-white transition-transform self-center',
...
onMounted(() => {
  console.log('[LoadBalancerGroupDialog] onMounted called, props.group:', props.group)
  if (props.group) {
...
const handleSubmit = () => {
  console.log('[LoadBalancerGroupDialog] handleSubmit called, formData:', formData)
  if (formData.toolIds.length === 0) {
    console.log('[LoadBalancerGroupDialog] No tools selected, alerting')
    alert('请至少选择一个工具')
    return
  }
  console.log('[LoadBalancerGroupDialog] Submitting and closing')
  emit('save', { ...formData })
  emit('close')
}

const handleOverlayClick = (event: MouseEvent) => {
  console.log('[LoadBalancerGroupDialog] Overlay clicked', event.target)
  // Stop propagation to prevent affecting the parent dialog
