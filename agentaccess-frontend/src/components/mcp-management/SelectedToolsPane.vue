<template>
  <div class="space-y-4">
    ...
    <div v-if="loadBalancerGroups.length > 0" class="space-y-3">
      <h4 class="text-sm font-medium text-gray-700">负载均衡组</h4>
    ...
    <div v-if="individualToolIds.length > 0" class="space-y-3">
      <h4 class="text-sm font-medium text-gray-700">已选工具 ({{ individualToolIds.length }})</h4>
    ...
    <!-- Empty State -->
    <div v-if="loadBalancerGroups.length === 0 && individualToolIds.length === 0" class="text-center py-8 text-gray-400 text-sm">
